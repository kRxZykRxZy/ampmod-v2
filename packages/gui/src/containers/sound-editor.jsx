...
class SoundEditor extends React.Component {
    ...
    tooLoud() {
        ...
        const startIndex =
            this.state.trimStart === null
                ? 0
                : Math.floor(this.state.trimStart * numChunks);
        const endIndex =
            this.state.trimEnd === null
                ? numChunks - 1
                : Math.ceil(this.state.trimEnd * numChunks);
    ...
    handleUndo() {
        ...
        const { samples, sampleRate, trimStart, trimEnd } =
            this.undoStack.pop();
        if (samples) {
...
const mapStateToProps = (state, { soundIndex }) => {
    ...
    const index =
        soundIndex < sprite.sounds.length
            ? soundIndex
            : sprite.sounds.length - 1;
